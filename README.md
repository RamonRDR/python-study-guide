...
The project foundation and five complete educational sections are available. [Phase 1: Fundamentals](fundamentals/README.md) provides six reviewed beginner chapters. Phase 6 contains six reviewed learning chapters:
...
Phases 1, 2, 3, 4, and 6 are complete. Phase 4 now contains eight reviewed Program Flow chapters: [Conditions, Comparisons, and Boolean Logic](program-flow/01-conditions-comparisons-and-boolean-logic/README.md) builds trustworthy conditions, [`if`, `elif`, and `else`](program-flow/02-if-elif-and-else/README.md) uses them for conditional branching, [`match` and `case`: Structural Pattern Matching](program-flow/03-match-and-case/README.md) adds pattern-based selection, [`for` Loops and Iteration](program-flow/04-for-loops-and-iteration/README.md) introduces iterable-driven repetition, [`range()`, `enumerate()`, and `zip()`](program-flow/05-range-enumerate-and-zip/README.md) adds numeric progressions, position-aware iteration, and parallel iteration, [`while` Loops and State-Driven Repetition](program-flow/06-while-loops-and-state-driven-repetition/README.md) adds repetition controlled by changing state, [`break`, `continue`, and Loop `else`](program-flow/07-break-continue-and-loop-else/README.md) adds deliberate loop control, and [Choosing and Combining Program Flow](program-flow/08-choosing-and-combining-program-flow/README.md) closes the phase with a decision framework for selecting and composing flow tools by intent. Phase 5: Functions is the next planned learning phase. Phase 3 contains six reviewed Collections chapters, ending with [Choosing the Right Collection](collections/06-choosing-the-right-collection/README.md). Phase 2 remains complete with four reviewed chapters, ending with [Numeric Built-ins](strings-and-numbers/04-numeric-builtins/README.md). See the [roadmap](docs/roadmap.en.md) or the [full learning path](docs/learning-path.en.md) for the current curriculum status and direct chapter links.
