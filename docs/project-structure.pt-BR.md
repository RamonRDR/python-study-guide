...
- `program-flow/`: trilha da Fase 4 em andamento. Os Capítulos 01–03 ensinam condições, comparações, teste de valor de verdade, pertencimento, identidade, lógica booleana, ramificação condicional com `if`, `elif` e `else` e correspondência de padrões estruturais com padrões literais, OR, de sequência, de mapping, de captura, curinga e guards em inglês, português brasileiro e espanhol, com exemplos executáveis determinísticos.
