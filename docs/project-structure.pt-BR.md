...
- `program-flow/`: trilha da Fase 4 em andamento. Os Capítulos 01–04 ensinam condições, comparações, teste de valor de verdade, pertencimento, identidade, lógica booleana, ramificação condicional com `if`, `elif` e `else`, correspondência de padrões estruturais e iteração item por item com `for` sobre listas, tuplas, strings, dicionários e conjuntos, em inglês, português brasileiro e espanhol, com exemplos executáveis determinísticos.
