...
- `functions/`: Phase 5 learning path in progress. Chapters 01–04 cover defining and calling functions, required inputs, returned values, local and global names, per-call local namespaces, lookup, shadowing, ordinary statement scope behavior, `NameError`, `UnboundLocalError`, and cautious use of `global`, in English, Brazilian Portuguese, and Spanish with deterministic executable examples.
