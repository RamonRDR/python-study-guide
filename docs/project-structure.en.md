...
- `practical-projects/`: Phase 10 practical-project workspace. Projects 01–06 are complete and Project 07 Fictional Reconciliation Workflow is in progress; Project 08 Simulated Automation Flow remains planned. Project 06 adds deterministic shallow discovery, immutable planning, collision policies, symlink boundaries, filesystem identity checks, descriptor-anchored directories, bounded staging names, atomic Linux no-replace commit behavior, a deterministic demo, and focused regression tests. Project 07 adds immutable reconciliation models, exact integer-cent arithmetic, deterministic matching and ordering, duplicate rejection, explicit reconciliation statuses, validated report invariants, stable text rendering, and focused precision and safety regression tests.
