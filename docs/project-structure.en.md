...
- `standard-library/`: in-progress Phase 8 learning path. Chapters 01–05 cover `pathlib` filesystem boundaries, `datetime` date/time modeling, advanced `json` serialization/decoding contracts, advanced `csv` tabular-text contracts, and advanced `logging` event-delivery contracts including configuration, contextual records, queues, concurrency, and operational safety, in English, Brazilian Portuguese, and Spanish with deterministic executable examples. Chapter 06 on `collections` is the next planned chapter.
