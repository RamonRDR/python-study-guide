...
- `program-flow/`: Phase 4 learning path in progress. Chapters 01–03 teach conditions, comparisons, truth-value testing, membership, identity, Boolean logic, conditional branching with `if`, `elif`, and `else`, and structural pattern matching with literal, OR, sequence, mapping, capture, wildcard, and guard patterns in English, Brazilian Portuguese, and Spanish with deterministic executable examples.
