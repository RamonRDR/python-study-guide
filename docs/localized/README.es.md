...
La base del proyecto y cinco secciones educativas completas están disponibles. La [Fase 1: Fundamentos](../../fundamentals/README.es.md) ofrece seis capítulos revisados para principiantes. La Fase 6 reúne seis capítulos de aprendizaje revisados:
...
Las Fases 1, 2, 3, 4 y 6 están completadas. La Fase 4 ahora reúne ocho capítulos revisados de Flujo del Programa: [Condiciones, Comparaciones y Lógica Booleana](../../program-flow/01-conditions-comparisons-and-boolean-logic/README.es.md) construye condiciones confiables, [`if`, `elif` y `else`](../../program-flow/02-if-elif-and-else/README.es.md) usa esas condiciones para ramificaciones condicionales, [`match` y `case`: Coincidencia de Patrones Estructurales](../../program-flow/03-match-and-case/README.es.md) añade selección mediante patrones, [Bucles `for` e Iteración](../../program-flow/04-for-loops-and-iteration/README.es.md) introduce repetición guiada por iterables, [`range()`, `enumerate()` y `zip()`](../../program-flow/05-range-enumerate-and-zip/README.es.md) añade progresiones numéricas, iteración con posición e iteración paralela, [Bucles `while` y Repetición Guiada por Estado](../../program-flow/06-while-loops-and-state-driven-repetition/README.es.md) añade repetición controlada por estado cambiante, [`break`, `continue` y `else` de Bucles](../../program-flow/07-break-continue-and-loop-else/README.es.md) añade control deliberado de bucles, y [Elegir y Combinar el Flujo del Programa](../../program-flow/08-choosing-and-combining-program-flow/README.es.md) cierra la fase con un modelo de decisión para seleccionar y combinar herramientas de flujo según la intención. La Fase 5: Funciones es la siguiente fase de aprendizaje planificada. La Fase 3 reúne seis capítulos revisados de Colecciones, terminando con [Elegir la Colección Adecuada](../../collections/06-choosing-the-right-collection/README.es.md). La Fase 2 permanece completada con cuatro capítulos revisados, terminando con [Funciones Numéricas Incorporadas](../../strings-and-numbers/04-numeric-builtins/README.es.md). Consulta el [roadmap](../roadmap.es.md) o la [ruta completa de aprendizaje](../learning-path.es.md) para seguir el estado del currículo y acceder directamente a los capítulos.
