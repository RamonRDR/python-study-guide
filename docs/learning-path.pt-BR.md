...
A Fase 8 está em andamento. O Capítulo 01 introduz objetos de caminho e fronteiras do filesystem. O Capítulo 02 acrescenta datas, horários, durações, parsing, formatação, datetimes naive versus aware, offsets UTC fixos, conversão de timezone e cálculos de tempo determinísticos. O Capítulo 03 aprofunda políticas de serialização e decodificação JSON, incluindo saída determinística, tratamento estrito de números não finitos, hooks numéricos, valores personalizados, nomes duplicados, diagnósticos e limites de recursos. O Capítulo 04 aprofunda CSV como fronteira de texto tabular com dialetos explícitos, quoting e escaping, política de formato das linhas, parsing estrito, limites de campos, sniffing heurístico, fronteiras de encoding e segurança para consumidores em planilhas. O Capítulo 05 aprofunda logging como um grafo de entrega de eventos em runtime com níveis efetivos, roteamento por handlers, propagação, `dictConfig()`, registros contextuais, atribuição do chamador, entrega por filas, concorrência e segurança de configuração. O próximo capítulo planejado é **`collections`**.
...
Planejado. Os links diretos dos projetos aparecerão aqui conforme essa fase for publicada.
