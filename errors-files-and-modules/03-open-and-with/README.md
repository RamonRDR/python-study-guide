...
Python's built-in `open()` function creates a **file object** for a filesystem path or an integer file descriptor. It does not wrap an arbitrary existing file-like object. The `with` statement gives the opened resource a clear lifetime so it is closed even when the block exits because of an exception.
...
|---|---|
| `FileNotFoundError` | the requested file or directory does not exist |
...
| Requested file or directory is missing | `FileNotFoundError` |
