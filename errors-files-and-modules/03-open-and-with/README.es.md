...
La función incorporada `open()` de Python crea un **objeto archivo** conectado a un archivo o a otro recurso similar a un archivo. La instrucción `with` da a ese recurso un tiempo de vida claro, de modo que se cierre incluso cuando el bloque termina debido a una excepción.
...
|---|---|
| `FileNotFoundError` | una ruta necesaria no existe |
