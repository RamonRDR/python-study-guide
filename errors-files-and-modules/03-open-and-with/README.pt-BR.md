...
A função embutida `open()` do Python cria um **objeto arquivo** para um caminho do sistema de arquivos ou para um descritor de arquivo inteiro. Ela não encapsula arbitrariamente um objeto semelhante a arquivo que já exista. A instrução `with` dá ao recurso aberto um tempo de vida claro, garantindo seu fechamento mesmo quando o bloco termina por causa de uma exceção.
...
|---|---|
| `FileNotFoundError` | o arquivo ou diretório solicitado não existe |
...
| Remover somente `\n` final | `line.rstrip("\n")` |
| Arquivo ou diretório solicitado ausente | `FileNotFoundError` |
